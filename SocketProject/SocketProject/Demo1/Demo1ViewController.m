...
#define SocketPort htons(8029)
//inet_addr是一个计算机函数，功能是将一个点分十进制的IP转换成一个长整数型数
#define SocketIP   inet_addr("127.0.0.1")

@interface Demo1ViewController ()
@property (nonatomic, assign) int clinenId;
@property (nonatomic, strong) UITextView *allMsgContent_tv;
@property (nonatomic, strong) UITextField *sendMsgContent_tf;
...
- (void)viewDidLoad {
    ...
    [btn setTitle:@"建立连接" forState:UIControlStateNormal];
    btn.backgroundColor = [UIColor brownColor];
    [btn addTarget:self action:@selector(socketConnetAction:) forControlEvents:UIControlEventTouchUpInside];
    
    
    UIButton *btn1 = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.view addSubview:btn1];
    btn1.frame = CGRectMake(100, 200, 150, 50);
    [btn1 setTitle:@"sendMsg" forState:UIControlStateNormal];
    btn1.backgroundColor = [UIColor brownColor];
    [btn1 addTarget:self action:@selector(sendMsgAction:) forControlEvents:UIControlEventTouchUpInside];
